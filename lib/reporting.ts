...
export function buildSchoolReport(data: AppData) {
  ...
  const body = `<div class="grid"><div class="card"><div>إجمالي الطلاب</div><div class="number">${data.students.length}</div></div><div class="card"><div>الصفوف</div><div class="number">${data.classes.length}</div></div><div class="card"><div>حضور اليوم</div><div class="number">${present}</div></div></div><h2 class="title">كشف الطلاب</h2><table><thead><tr><th>الرقم</th><th>اسم الطالب</th><th>الصف والشعبة</th><th>السلوك</th><th>حالة الطالب</th></tr></thead><tbody>${data.students.map((student) => { const summary = calculateBehaviorSummary(data, student.id); const behaviorStatus = summary.risk === "dismissed" ? "مفصول" : summary.risk === "warning" ? "تنبيه" : "سليم"; return `<tr><td>${safe(student.studentNumber)}</td><td>${safe(student.fullName)}</td><td>${safe(className(data, student))}</td><td>${summary.totalPoints} / ${summary.dismissalThreshold} (${behaviorStatus})</td><td>${safe(student.status)}</td></tr>`; }).join("") || "<tr><td colspan='5'>لا توجد بيانات طلاب مسجلة.</td></tr>"}</tbody></table>${dismissed ? `<p><strong>تنبيه:</strong> يوجد ${dismissed} طالب بلغوا حد الفصل السلوكي.</p>` : ""}`;
