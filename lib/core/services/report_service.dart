...
class ReportService {
  ...
  pw.Widget _pdfSummaryCards(List<List<String>> values) => pw.Row(children: [for (var index = 0; index < values.length; index++) pw.Expanded(child: pw.Container(margin: const pw.EdgeInsets.all(3), padding: const pw.EdgeInsets.all(8), decoration: const pw.BoxDecoration(color: PdfColors.blue50), child: pw.Column(children: [pw.Text(values[index][0], style: const pw.TextStyle(fontSize: 9)), const pw.SizedBox(height: 3), pw.Text(values[index][1], style: const pw.TextStyle(fontSize: 16, fontWeight: pw.FontWeight.bold))]))) ]);
