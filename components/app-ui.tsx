...
export const colors = {
  navy: appTheme.colors.primaryDark,
  blue: appTheme.colors.primary,
  blueSurface: appTheme.colors.primarySurface,
  pale: appTheme.colors.background,
  ink: appTheme.colors.ink,
  muted: appTheme.colors.muted,
  border: appTheme.colors.border,
  success: appTheme.colors.success,
  successSurface: appTheme.colors.successSurface,
  warning: appTheme.colors.warning,
  danger: appTheme.colors.danger,
  dangerSurface: appTheme.colors.dangerSurface,
  white: appTheme.colors.surface,
  overlay: appTheme.colors.overlay,
} as const;

const touchFeedback = { color: appTheme.highlightColor } as const;
...
export function PrimaryButton({ label, icon, onPress, disabled, compact = false }: { label: string; icon?: keyof typeof MaterialIcons.glyphMap; onPress: () => void; disabled?: boolean; compact?: boolean }) {
  return <Pressable accessibilityRole="button" accessibilityState={{ disabled: Boolean(disabled) }} disabled={disabled} onPress={onPress} android_ripple={touchFeedback} style={({ pressed }) => [styles.primary, compact && styles.compact, (pressed || disabled) && styles.dim, disabled && styles.disabled]}><Text style={styles.primaryText} numberOfLines={1}>{label}</Text>{icon ? <MaterialIcons name={icon} color={colors.navy} size={20} /> : null}</Pressable>;
}
...
export function SecondaryButton({ label, icon, onPress, danger = false }: { label: string; icon?: keyof typeof MaterialIcons.glyphMap; onPress: () => void; danger?: boolean }) {
  return <Pressable accessibilityRole="button" onPress={onPress} android_ripple={touchFeedback} style={({ pressed }) => [styles.secondary, danger && styles.dangerOutline, pressed && styles.dim]}>{icon ? <MaterialIcons name={icon} color={danger ? colors.danger : colors.navy} size={20} /> : null}<Text style={[styles.secondaryText, danger && styles.dangerText]} numberOfLines={1}>{label}</Text></Pressable>;
}
...
export function ChoicePill({ label, selected, onPress }: { label: string; selected: boolean; onPress: () => void }) {
  return <Pressable accessibilityRole="button" accessibilityState={{ selected }} onPress={onPress} android_ripple={touchFeedback} style={({ pressed }) => [styles.pill, selected && styles.pillSelected, pressed && styles.dim]}><Text style={[styles.pillText, selected && styles.pillTextSelected]} numberOfLines={1}>{label}</Text></Pressable>;
}
...
export function Sheet({ visible, title, onClose, children }: { visible: boolean; title: string; onClose: () => void; children: ReactNode }) {
  return <Modal animationType="slide" transparent visible={visible} statusBarTranslucent onRequestClose={onClose}><KeyboardAvoidingView style={styles.modalBackdrop} behavior={Platform.select({ ios: "padding", android: "height" })}><SafeAreaView style={styles.sheet} edges={["bottom"]}><View style={styles.sheetHeader}><Text style={styles.sheetTitle} numberOfLines={1}>{title}</Text><Pressable accessibilityRole="button" accessibilityLabel="إغلاق" onPress={onClose} android_ripple={touchFeedback} style={styles.close}><MaterialIcons name="close" size={22} color={colors.ink} /></Pressable></View><ScrollView style={styles.sheetContent} contentContainerStyle={styles.sheetScrollContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>{children}</ScrollView></SafeAreaView></KeyboardAvoidingView></Modal>;
}

export function LoadingScreen() { return <SafeAreaView style={styles.loading}><ActivityIndicator size="large" color={colors.blue} /><Text style={styles.loadingText}>يتم تجهيز بيانات المدرسة...</Text></SafeAreaView>; }

const styles = StyleSheet.create({
  header: { flexDirection: "row-reverse", justifyContent: "space-between", alignItems: "flex-start", columnGap: 14, marginBottom: 22 }, headerText: { flex: 1, minWidth: 0, alignItems: "flex-end" }, headerAction: { flexShrink: 0 },
  ...
  primary: { backgroundColor: colors.blueSurface, borderColor: colors.blue, borderWidth: 1.5, minHeight: 52, paddingHorizontal: 18, borderRadius: 14, flexDirection: "row-reverse", alignItems: "center", justifyContent: "center", columnGap: 8, alignSelf: "stretch", overflow: "hidden" }, compact: { minHeight: 44, paddingHorizontal: 14, alignSelf: "auto" }, primaryText: { color: colors.navy, fontSize: 16, lineHeight: 23, fontWeight: "800", textAlign: "center", flexShrink: 1 }, disabled: { backgroundColor: "#E5EBF2", borderColor: "#C9D6E3" }, dim: { opacity: 0.72, transform: [{ scale: 0.985 }] },
  secondary: { minHeight: 46, borderRadius: 13, paddingHorizontal: 14, borderWidth: 1, borderColor: "#9DBCE2", backgroundColor: "#EDF5FF", flexDirection: "row-reverse", alignItems: "center", justifyContent: "center", columnGap: 7, flexShrink: 1, overflow: "hidden" }, secondaryText: { color: colors.navy, fontWeight: "800", fontSize: 15, lineHeight: 21, textAlign: "center", flexShrink: 1 }, dangerOutline: { borderColor: "#E6A8AE", backgroundColor: colors.dangerSurface }, dangerText: { color: colors.danger },
