...
## تصحيح لاحق: الانعكاس المزدوج في Excel

النقطتان 4 و5 أعلاه بنت خطتهما على افتراض أن `excel_plus` لا يدعم اتجاه ورقة
RTL، فاختارتا عكس الأعمدة فيزيائياً. هذا الافتراض غير دقيق:

- `Sheet.isRTL` في `excel_plus` (وفي `excel` التي هي بديلها المتوافق) تُترجم
  إلى `<sheetView rightToLeft="1"/>` في ملف الورقة، وهو ما يجعل Excel يعرض
  العمود A في أقصى اليمين والعمود B إلى يساره وهكذا.
- توثيق الخاصية في pub.dev يذكر صراحة أنها تجعل الورقة «right-to-left»،
  ومصدر الحزمة يربط Setter بقيمة `_rtlChangeLookup` التي يستهلكها الكاتب.
- `ExcelReportBuilder.addSheet` كان يضبط `isRTL = true` ويعكس الأعمدة معاً،
  فحصل انعكاس مزدوج: العمود A يحمل «هاتف ولي الأمر» ويُعرض في أقصى اليمين،
  فيقرأ المستخدم الجدول من اليسار إلى اليمين ويظهر عمود «ت» والاسم الكامل في
  أقصى اليسار.

القرار المعتمد الآن: `isRTL = true` هو المسؤول الوحيد عن الاتجاه، وتُكتب
الترويسة والقيم وعروض الأعمدة بالترتيب المنطقي دون عكس فيزيائي. هذا يجعل
البيانات المخزّنة في الملف مطابقة للترتيب المنطقي، فتقرأها الأدوات الأخرى
(ومنها استيراد الطلاب في التطبيق) بصورة صحيحة.

عكس الرؤوس والصفوف في جداول PDF يبقى مقصوداً وصحيحاً، لأن `TableHelper.fromTextArray`
لا يعكس الخلايا تلقائياً كما وثّقت نقطة «تنفيذ PDF الداخلي» أعلاه.

التحقق: أعيد بناء عينتي XLSX بنفس منطق الباني قبل الإصلاح وبعده (ورقة واحدة،
تسعة أعمدة، `rightToLeft="1"`) وقورن الموضع البصري لعمود «الاسم الكامل»:

| العيّنة | أقصى اليمين | موضع «الاسم الكامل» |
| --- | --- | --- |
| قبل الإصلاح (عكس فيزيائي + RTL) | هاتف ولي الأمر | العمود الثاني من اليسار |
| بعد الإصلاح (ترتيب منطقي + RTL) | ت | العمود الثاني من اليمين |
