...
export default function StudentsScreen() {
  ...
  return (
    <ScreenContainer className="p-4">
      <ScreenHeader title="الطلاب" subtitle={`${data.students.length} طالبًا مسجلًا`} action={<PrimaryButton label="إضافة" icon="person-add" onPress={() => setFormOpen(true)} compact />} />
      ...
      <View style={styles.searchRow}>
        <Pressable accessibilityRole="button" accessibilityLabel="تصفية الطلاب" onPress={() => setFilterOpen(true)} style={styles.filter}>
      ...
      <Sheet visible={filterOpen} title="تصفية الطلاب" onClose={() => setFilterOpen(false)}>
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.choices}>
            ...
            <ChoicePill label={`غيابات متكررة (${REPEATED_ABSENCE_THRESHOLD}+)`} selected={attentionFilter === "repeated-absence"} onPress={() => setAttentionFilter("repeated-absence")} />
          </View>
...
function StudentListTile({ item, index }: { item: Student; index: number }) {
  ...
  return (
    <Pressable accessibilityRole="button" accessibilityLabel={`الطالب ${index + 1}: ${item.fullName}`} onPress={() => router.push(`/student/${item.id}` as any)} style={({ pressed }) => [styles.listTile, pressed && styles.pressed]}>
      ...
      <View style={styles.tileText}>
        ...
        <Text style={[styles.studentSubtitle, behavior.risk !== "clear" && styles.warningSubtitle]} numberOfLines={1}>السلوك: {behavior.totalPoints} نقطة · الغيابات: {absences}</Text>
      </View>
...
const styles = StyleSheet.create({
  ...
  filter: { width: 48, height: 48, borderRadius: 14, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.white, alignItems: "center", justifyContent: "center" },
  ...
  filterText: { flex: 1, color: colors.muted, fontSize: 14, lineHeight: 20 },
  sortLabel: { color: colors.navy, fontSize: 13, fontWeight: "800" },
  studentList: { flex: 1 },
  listContent: { rowGap: 9, paddingBottom: 112 },
  listTile: { minHeight: 78, paddingHorizontal: 14, paddingVertical: 10, borderRadius: 16, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.white, flexDirection: "row", alignItems: "center", columnGap: 12 },
  ...
  tileText: { flex: 1, minWidth: 0, rowGap: 3 },
  studentName: { color: colors.ink, fontSize: 16, lineHeight: 23, fontWeight: "800" },
  studentSubtitle: { color: colors.muted, fontSize: 12, lineHeight: 18 },
  warningSubtitle: { color: colors.danger, fontWeight: "700" },
  pressed: { opacity: 0.72 },
  emptyWrap: { flex: 1, justifyContent: "center", paddingBottom: 112 },
  sheetHint: { color: colors.muted, fontSize: 15, lineHeight: 22, marginBottom: 12 },
  choiceLabel: { color: colors.ink, fontSize: 15, fontWeight: "800", marginBottom: 8 },
  ...
  formError: { color: colors.danger, fontSize: 14, lineHeight: 20, marginTop: -9, marginBottom: 13 },
});
